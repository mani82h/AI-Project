...
from main import make_move, get_possible_moves, print_cards_status, set_banners, calculate_winner
    ...
    for card in cards:
        ...
        if banners.get(house) + 1 >= opponent_banners.get(house):
            ...
            if banners.get(house) + 1 >= (count // 2 + 1):
                ...
                score += config.weights["capture_banner_bonus"]
        ...
        for neighbor_loc in neighbors:
            ...
            if neighbor_card and neighbor_card.get_house() == house:
                score += config.weights["row_col_priority"]
        ...
        if house in house_variances:
            score -= house_variances[house] * config.weights["house_variance_weight"]
...
def getScore(cards, player1, player2, turn):
    if len(cards) == 0:
        ...
        if turn == 1:
            return config.weights["who_has_more"] * who_has_more(player1, player2) + config.weights["banner_difference_score"] * banner_difference_score(player1, player2) + config.weights["heuristic"] * heuristic(player1, player2, cards, varys_location)
        else:
            return -config.weights["who_has_more"] * who_has_more(player2, player1) + -config.weights["banner_difference_score"] * banner_difference_score(player2, player1) - config.weights["heuristic"] * heuristic(player2, player1, cards, varys_location)
